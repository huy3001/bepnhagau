...
/*-----------------------------------------------------------------------------------*/
define( 'MTS_THEME_NAME', 'myportfolio' );
define( 'MTS_THEME_VERSION', '1.1.0' );

require_once( dirname( __FILE__ ) . '/theme-options.php' );
if ( ! isset( $content_width ) ) {
	$content_width = 778; //article content width without padding
}
...
function mts_after_setup_theme() {
	...
	load_theme_textdomain( 'myportfolio', get_template_directory().'/lang' );

	add_theme_support( 'post-thumbnails' );
	set_post_thumbnail_size( 355, 250, true );
	add_image_size( 'myportfolio-featured', 355, 250, true ); // blog featured thumbs
	add_image_size( 'myportfolio-featuredfull', 770, 500, true ); // blog featured for single posts
	add_image_size( 'myportfolio-fullwidth', 1130, 700, true ); // blog featured full width
	add_image_size( 'myportfolio-portfolio', 366, 366, true ); // portfolio
	add_image_size( 'myportfolio-widgetthumb', 72, 61, true ); // widget
	add_image_size( 'myportfolio-widgetfull', 285, 190, true ); // widget full width

	register_nav_menus( array(
	  'primary-menu' => __( 'Primary', 'myportfolio' ),
	  'footer-menu' => __( 'Footer', 'myportfolio' ),
	  'mobile' => __( 'Mobile', 'myportfolio' )
	) );
...
function mts_posttype_register() {
	...
	//Portfolio Post type
	$args = array(
		'label' => __('Portfolio', 'myportfolio'),
		'singular_label' => __('Portfolio', 'myportfolio'),
		'public' => true,
		'show_ui' => true,
	...
	//Portfolio Taxonomy
	$arr = array("hierarchical" => true, "Categories"   => array('singular_name' => 'Category'), "show_admin_column" => true, 'rewrite' => array("slug" => "portfolio-category"));
...
if (isset($_GET['activated']) && is_admin()){
	//Blog
	$blog_page_title = __('Blog','myportfolio');
	$blog_page_content = '';
...
function mts_get_icons() {
	$icons = array(
		__( 'Web Application Icons', 'myportfolio' ) => array(
		...
		__( 'Accessibility Icons', 'myportfolio' ) => array(
		...
		__( 'Hand Icons', 'myportfolio' ) => array(
		...
		__( 'File Type Icons', 'myportfolio' ) => array(
		...
		__( 'Spinner Icons', 'myportfolio' ) => array(
			'circle-o-notch', 'cog', 'refresh', 'spinner'
		),
		__( 'Form Control Icons', 'myportfolio' ) => array(
		...
		__( 'Payment Icons', 'myportfolio' ) => array(
		...
		__( 'Chart Icons', 'myportfolio' ) => array(
			'area-chart', 'bar-chart', 'line-chart', 'pie-chart'
		),
		__( 'Currency Icons', 'myportfolio' ) => array(
			'btc', 'eur', 'gbp', 'gg', 'gg-circle', 'ils', 'inr', 'jpy', 'krw', 'money', 'rub', 'try', 'usd'
		),
		__( 'Text Editor Icons', 'myportfolio' ) => array(
		...
		__( 'Directional Icons', 'myportfolio' ) => array(
		...
		__( 'Video Player Icons', 'myportfolio' ) => array(
		...
		__( 'Brand Icons', 'myportfolio' ) => array(
		...
		__( 'Medical Icons', 'myportfolio' ) => array(
...
function mts_get_featured_image($post_ID) {  
	...
	if ($post_thumbnail_id) {  
		$post_thumbnail_img = wp_get_attachment_image_src($post_thumbnail_id, 'myportfolio-widgetfull');  
...
function mts_columns_head($defaults) {
	if (get_post_type() == 'post' || get_post_type() == 'portfolio') {
		$defaults['featured_image'] = __('Featured Image', 'myportfolio' );
	}
...
function mts_register_sidebars() {
	...
	register_sidebar( array(
		'name' => __('Sidebar', 'myportfolio'),
		'description'   => __( 'Default sidebar.', 'myportfolio' ),
		'id' => 'sidebar',
	...
	// Portfolio sidebar
	register_sidebar( array(
		'name' => __('Portfolio Sidebar', 'myportfolio'),
		'description'   => __( 'Portfolio single page sidebar.', 'myportfolio' ),
	...
	if ( mts_is_wc_active() ) {
		// Register WooCommerce Single Product Sidebar
		register_sidebar( array(
			'name' => __('Shop Page Sidebar', 'myportfolio' ),
			'description'   => __( 'Appears on Shop main page and product archive pages.', 'myportfolio' ),
			'id' => 'shop-sidebar',
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget' => '</div>',
			'before_title' => '<h3 class="widget-title">',
		...
		register_sidebar( array(
			'name' => __('Single Product Sidebar', 'myportfolio' ),
			'description'   => __( 'Appears on single product pages.', 'myportfolio' ),
			'id' => 'product-sidebar',
			'before_widget' => '<div id="%1$s" class="widget %2$s">',
			'after_widget' => '</div>',
			'before_title' => '<h3 class="widget-title">',
...
function mts_custom_sidebar() {
	...
	if ( is_singular('portfolio')) {
		...
		if ( !empty( $mts_options['mts_sidebar_for_portfolio_post'] )) {
			$sidebar = $mts_options['mts_sidebar_for_portfolio_post'];
		}
	}
...
// About Author widget.
include_once( 'functions/widget-about-author.php' );
...
function mts_add_scripts() {
	...
	wp_register_script( 'customscript', get_template_directory_uri() . '/js/customscript.js', array('jquery', 'modernizr'), null, true );
...
function mts_load_footer_scripts() {  
	...
	if ( ! empty( $mts_options['mts_lazy_load'] ) ) {
		if ( ! empty( $mts_options['mts_lazy_load_thumbs'] ) || ( ! empty( $mts_options['mts_lazy_load_content'] ) && is_singular() ) ) {
			wp_enqueue_script( 'layzr', get_template_directory_uri() . '/js/layzr.min.js', array('jquery'), null, true );
		}
	}

	// Ajax Load More and Search Results
	wp_register_script( 'mts_ajax', get_template_directory_uri() . '/js/ajax.js', array('jquery'), null, true );

	if ( is_home() ) {
		$pagenavigation_type = 'mts_pagenavigation_type';
	} else {
		$pagenavigation_type = 'mts_blog_pagenavigation_type';
	}
	
	if ( isset($mts_options[$pagenavigation_type]) && !empty( $mts_options[$pagenavigation_type] ) && $mts_options[$pagenavigation_type] >= 2 && ( !is_singular() || is_page_template('page-blog.php') ) ) {
		...
		wp_localize_script(
			...
			array(
				...
				'autoLoad' => $autoload,
				'i18n_loadmore' => __( 'Load More Posts', 'myportfolio' ),
				'i18n_loading' => __('Loading...', 'myportfolio' ),
				'i18n_nomore' => __( 'No more posts.', 'myportfolio' )
			 )
		);
	}
...
function mts_enqueue_css() {
	...
	$mts_home_icon = '';
	$mts_single_portfolio_fullwidth = '';
	$mts_comment_form = '';
	$mts_breadcrumb = '';
	...
	if ( $mts_sidebar_location != 'right' && ( $mts_options['mts_layout'] == 'sclayout' || $mts_sidebar_location == 'left' )) {
		$mts_sclayout = '.article { float: right; padding-left: 30px; }
		.sidebar.c-4-12 { float: left; padding-right: 0; padding-left: 30px; }';
	...
	if( $mts_options['mts_lightbox'] == 0 ) {
		$mts_home_icon = '#home-page .icons, .similar-projects .icons { width: 38px; margin-left: -16px; }';
	}
	$entries = get_post_meta( get_the_ID(), '_mts_portfolio_info_group', true );
	if ( empty( $entries ) ) {
		$mts_single_portfolio_fullwidth = '.single-portfolio-full .left-half, .single-portfolio-full .right-half { width: 100%; max-width: 100%; } ';
	}
	$comment_on = is_singular() && comments_open();
	if( !$comment_on ) {
		$mts_comment_form = ' #comments { margin-bottom: 30px } ';
	}
	if ( empty( $mts_options['mts_header_section2'] ) ) {
		$mts_header_section = '.logo-wrap, .widget-header { display: none!important; }
		.navigation { border-top: 0; }
		#header { min-height: 47px; }
		';
	}
	if ( ! empty( $mts_options['mts_author_comment'] ) ) {
		$mts_author = '.bypostauthor .fn:after { content: "'.__( 'Author', 'myportfolio' ).'"; position: relative; margin-left: 5px; padding: 3px 7px; color: #ffffff; font-size: 12px; }';
	}
	if( $mts_options['mts_breadcrumb'] == 0 ) {
		$mts_breadcrumb = '.sidebar.c-4-12 { margin-top: 28px; }
		.single_post, .single_page { margin-top: 0; }';
	}
			...
			switch( $comment->comment_type ) :
				...
				case 'trackback': ?>
					<div id="comment-<?php comment_ID(); ?>">
						<div class="comment-author vcard">
							<?php _e('Pingback:','myportfolio'); ?> <?php comment_author_link(); ?>
							<?php if ( ! empty( $mts_options['mts_comment_date'] ) ) { ?>
								<span class="ago"><?php comment_date( get_option( 'date_format' ) ); ?></span>
							<?php } ?>
							<span class="comment-meta">
								<?php edit_comment_link( __( '( Edit )', 'myportfolio' ), '  ', '' ) ?>
							</span>
						</div>
						<?php if ( $comment->comment_approved == '0' ) : ?>
							<em><?php _e( 'Your comment is awaiting moderation.', 'myportfolio' ) ?></em>
							<br />
						<?php endif; ?>
					</div>
				<?php
				...
				default: ?>
					<div id="comment-<?php comment_ID(); ?>" itemscope itemtype="http://schema.org/UserComments">
						<div class="comment-author vcard">
							<?php echo get_avatar( $comment->comment_author_email, 71 ); ?>
	...
	function mts_readmore() {
		...
		<span class="readMore">
			<a href="<?php echo esc_url( get_the_permalink() ); ?>" title="<?php echo esc_attr( get_the_title() ); ?>">
				<?php _e( 'Full Article ', 'myportfolio' ); ?> <i class="fa fa-angle-right"></i>
			</a>
		</span>
		<?php 
	}
...
function ajax_mts_search() {
	...
	if ( !empty( $query ) && $search_query->have_posts() ) : 
			...
			?><li>
				<a href="<?php echo esc_url( get_the_permalink() ); ?>">
					<?php if ( has_post_thumbnail() ) { ?>
						<?php the_post_thumbnail( 'myportfolio-widgetthumb', array( 'title' => '' ) ); ?>
					<?php } else { ?>
		...
		endwhile;
		echo '</ul>';
		echo '<div class="ajax-search-meta"><span class="results-count">'.$search_count.' '.__( 'Results', 'myportfolio' ).'</span><a href="'.esc_url( get_search_link( $query ) ).'" class="results-link">'.__('Show all results.', 'myportfolio' ).'</a></div>';
	else:
		echo '<div class="no-results">'.__( 'No results found.', 'myportfolio' ).'</div>';
	endif;
	wp_reset_postdata();
...
function mts_custom_gravatar( $avatar_defaults ) {
	...
	$avatar_defaults[$mts_avatar] = __( 'Custom Gravatar ( /images/gravatar.png )', 'myportfolio' );
...
function mts_megamenu_thumbnails( $thumbnail_html, $post_id ) {
	...
	if(has_post_thumbnail($post_id)):
		$thumbnail_html .= get_the_post_thumbnail($post_id, 'myportfolio-widgetfull', array('title' => ''));
	else:
		$thumbnail_html .= '<img src="'.get_template_directory_uri().'/images/nothumb-myportfolio-widgetfull.png" alt="'.__('No Preview', 'myportfolio').'"  class="wp-post-image" />';
	endif;
	$thumbnail_html .= '</a>';
...
function mts_admin_bar_link() {
	...
	if( current_user_can( 'edit_theme_options' ) ) {
		$wp_admin_bar->add_menu( array(
			...
			'title' => __( 'Theme Options', 'myportfolio' ),
...
// Add blog/portfolio loading classes to body
add_filter( 'body_class', 'mts_loader_body_classes' );
function mts_loader_body_classes( $classes ) {

	$mts_options = get_option( MTS_THEME_NAME );

	if ( '1' == $mts_options['mts_loading'] ) {
		if ( is_singular( 'post' ) || is_archive() || is_search() || is_page_template( 'page-blog.php' ) ) {
			$classes[] = 'mts-loading-active';
			$classes[] = 'mts-blog-loading-active';
		} else if ( is_singular( 'portfolio' ) || is_home() ) {
			$classes[] = 'mts-loading-active';
			$classes[] = 'mts-portfolio-loading-active';
		}
	}
...
/*
 * Show portfolio items on homepage
 */
function mts_home_portfolio_query( $query ) {

	if ( ! $query->is_admin() && $query->is_home() && $query->is_main_query() && 'posts' === get_option('show_on_front') ) {
		...
		if ( isset( $mts_options['mts_portfolio_postsnum'] ) && !empty( $mts_options['mts_portfolio_postsnum'] ) ) {
			$query->set( 'posts_per_page', $mts_options['mts_portfolio_postsnum'] );
		}
	}
}
add_action( 'pre_get_posts', 'mts_home_portfolio_query' );

function update_cmb2_meta_box_url( $url ) {
    return get_stylesheet_directory_uri().'/functions/CMB2' ;
}
...
function mts_correct_homepage_sections_import( $item, $key, $data ) {
    ...
    if ( !in_array( $key, array( 'mts_myclients_images', 'mts_featured_categories' ) ) ) return $item;

    $new_item = $item;

    if ( 'mts_myclients_images' === $key ) {

        foreach( $item as $i => $image ) {

            $id = $image['mts_myclients_image'];

            if ( is_numeric( $id ) ) {

                if ( array_key_exists( $id, $data['posts'] ) ) {

                    $new_item[ $i ]['mts_myclients_image'] = $data['posts'][ $id ];
                }
                ...
                if ( array_key_exists( $id, $data['image_urls'] ) ) {
                    ...
                    $new_item[ $i ]['mts_myclients_image'] = $data['image_urls'][ $id ];
                }
            }
        }
