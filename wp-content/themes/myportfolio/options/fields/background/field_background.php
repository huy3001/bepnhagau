...
class NHP_Options_background extends NHP_Options{
	...
	*/
	function render(){
		...
		$css_options = array(
			'repeat'	 => array(
				''		  => __('Default', 'myportfolio' ),
				'no-repeat' => __('No Repeat', 'myportfolio' ),
				'repeat'	=> __('Repeat All', 'myportfolio' ),
				'repeat-x'  => __('Repeat Horizontally', 'myportfolio' ),
				'repeat-y'  => __('Repeat Vertically', 'myportfolio' ),
				//'inherit'   => 'Inherit',
			),
			'attachment' => array(
				''		=> __('Default', 'myportfolio' ),
				'fixed'   => __('Fixed', 'myportfolio' ),
				'scroll'  => __('Scroll', 'myportfolio' ),
				//'inherit' => 'Inherit',
			),
			'position'   => array(
				''			  => __('Default', 'myportfolio' ),
				'left top'	  => __('Left Top', 'myportfolio' ),
				'left center'   => __('Left center', 'myportfolio' ),
				'left bottom'   => __('Left Bottom', 'myportfolio' ),
				'center top'	=> __('Center Top', 'myportfolio' ),
				'center center' => __('Center Center', 'myportfolio' ),
				'center bottom' => __('Center Bottom', 'myportfolio' ),
				'right top'	 => __('Right Top', 'myportfolio' ),
				'right center'  => __('Right center', 'myportfolio' ),
				'right bottom'  => __('Right Bottom', 'myportfolio' ),
			),
			'size'	   => array(
				''		=> __('Default', 'myportfolio' ),
				//'inherit' => 'Inherit',
				'cover'   => __('Cover', 'myportfolio' ),
				'contain' => __('Contain', 'myportfolio' ),
			),
			'parallax'   => array(
				'0' => __('Off', 'myportfolio' ),
				'1' => __('On', 'myportfolio' ),
			),
		);
		...
		if ( $this->field['options']['color'] !== false ) {
			...
			echo '<div class="bg-opt-input-label">'.__( 'Background Color:', 'myportfolio' ).'</div>';
		...
		// Tabs
		echo '<div class="bg-opt-input-label">'.__( 'Background Image:', 'myportfolio' ).'</div>';
		echo '<fieldset class="green buttonset buttonset-tabs">';
			if ( $this->field['options']['image_pattern'] !== false ) {
				...
				echo '<label id="nhp-opts-button" for="'.$this->field['id'].'_pattern" class="buttonset-tab">'.__('Pattern', 'myportfolio' ).'</label>';
			}
			if ( $this->field['options']['image_upload'] !== false ) {
				...
				echo '<label id="nhp-opts-button" for="'.$this->field['id'].'_upload" class="buttonset-tab">'.__('Upload', 'myportfolio' ).'</label>';
			}
			if ( $this->field['options']['gradient'] !== false ) {
				...
				echo '<label id="nhp-opts-button" for="'.$this->field['id'].'_gradient" class="buttonset-tab">'.__('Gradient', 'myportfolio' ).'</label>';
			}
		...
		if ( $this->field['options']['image_upload'] !== false ) {
			...
			echo ' <a href="javascript:void(0);" class="nhp-opts-upload button-secondary"'.$upload.' rel-id="'.$this->field['id'].'_image_upload">'.__('Browse', 'myportfolio' ).'</a>';
			echo ' <a href="javascript:void(0);" class="nhp-opts-upload-remove"'.$remove.' rel-id="'.$this->field['id'].'_image_upload">'.__('Remove Upload', 'myportfolio' ).'</a>';
			echo '</fieldset>';
			...
			if ( $this->field['options']['repeat'] !== false ) {
				...
				echo '<label for="'.$this->field['id'].'_repeat" class="bg-opt-input-label">'.__( 'Background Repeat:', 'myportfolio' ).'</label>';
			...
			if ( $this->field['options']['attachment'] !== false ) {
				...
				echo '<div class="bg-upload-select">';
				echo '<label for="'.$this->field['id'].'_attachment" class="bg-opt-input-label">'.__( 'Background Attachment:', 'myportfolio' ).'</label>';
			...
			if ( $this->field['options']['position'] !== false ) {
				...
				echo '<div class="bg-upload-select">';
				echo '<label for="'.$this->field['id'].'_position" class="bg-opt-input-label">'.__( 'Background Position:', 'myportfolio' ).'</label>';
			...
			if ( $this->field['options']['size'] !== false ) {
				...
				echo '<div class="bg-upload-select">';
				echo '<label for="'.$this->field['id'].'_size" class="bg-opt-input-label">'.__( 'Background Size:', 'myportfolio' ).'</label>';
		...
		if ( $this->field['options']['gradient'] !== false ) {
			...
			echo '<div class="bg-opt-input-label">' . __('From:', 'myportfolio' ) . '</div><input type="text" id="'.$this->field['id'].'_gradient_from" name="'.$this->args['opt_name'].'['.$this->field['id'].'][gradient][from]" value="'.$this->value['gradient']['from'].'" class="popup-colorpicker" data-default-color="'.$defaults['gradient']['from'].'" style="width:70px;"/>';
			echo '</div>';
			echo '<div class="color-gradient-step-wrapper">';
			echo '<div class="bg-opt-input-label">' . __('To:', 'myportfolio' ) . '</div><input type="text" id="'.$this->field['id'].'_gradient_to" name="'.$this->args['opt_name'].'['.$this->field['id'].'][gradient][to]" value="'.$this->value['gradient']['to'].'" class="popup-colorpicker" data-default-color="'.$defaults['gradient']['to'].'" style="width:70px;"/>';
			echo '</div>';

			echo '<div class="color-gradient-direction-wrapper">';
			echo '<label for="'.$this->field['id'].'_gradient_direction" class="bg-opt-input-label">' . __('Direction:', 'myportfolio' ) . '</label>';
			echo '<select id="'.$this->field['id'].'_gradient_direction" name="'.$this->args['opt_name'].'['.$this->field['id'].'][gradient][direction]" class="color-gradient-direction">';
				echo '<option value="horizontal"' . selected( $this->value['gradient']['direction'], 'horizontal', false ) . '>' . __( 'Horizontal', 'myportfolio' ) . '</option>';
				echo '<option value="vertical"' . selected( $this->value['gradient']['direction'], 'vertical', false ) . '>' . __( 'Vertical', 'myportfolio' ) . '</option>';
			echo '</select>';
			echo '</div>';
			echo '</div>';
			echo '</div>';
		...
		if ( $this->field['options']['parallax'] !== false ) {
			...
			echo '<div class="bg-opt-input-label">'.__( 'Parallax Effect:', 'myportfolio' ).'</div>';
