...
class mtsGoogleTypography {
	...
	function help_tab() {
		...
		$adding_title				= __('Adding A Collection', 'myportfolio' );
		$adding_content				= '<p>'.__('To add a new font for use on your site. Click the "Add New" button on the top left of the page near the "Google Typography" title.', 'myportfolio' ).'</p>';
		$adding_content				.= '<p>'.__('Once added, a new font row will appear on the page below. Next you can continue to customize your font (more info in the "Customizing" help tab).', 'myportfolio' ).'</p>';
		//$adding_content			 .= '<p><a href="https://vimeo.com/67957799" target="_blank">'.__('Watch The Video Tutorial &rarr;', 'myportfolio' ).'</a></p>';
		$customizing_title 		= __('Customizing', 'myportfolio' );
		$customizing_content	= '<p>'.__('Customizing fonts is easy; after adding a new font row you can then customize the following font attributes:', 'myportfolio' ).'</p>';
		$customizing_content .= '<ul><li><b>'.__('Preview Text', 'myportfolio' ).'</b> - '.__('Used for live previewing your changes. This text does not appear anywhere on your website.', 'myportfolio' ).'</li><li><b>Preview Background Color</b> - Allows you to swap between light and dark backgrounds when previewing this font.</li><li><b>'.__('Font Family', 'myportfolio' ).'</b> - '.__('The font family to use for this font. Choose from a real-time list of all available Google Fonts.', 'myportfolio' ).'</li><li><b>'.__('Font Variant', 'myportfolio' ).'</b> - '.__('The variant to use for this font. Note: Each font has it\'s own variant options.', 'myportfolio' ).'</li><li><b>'.__('Font Size', 'myportfolio' ).'</b> - '.__('The size you would like this font to be.', 'myportfolio' ).'</li><li><b>'.__('Font Color', 'myportfolio' ).'</b> - '.__('The color you\'d like to use for this font.', 'myportfolio' ).'</li><li><b>'.__('CSS Selectors', 'myportfolio' ).'</b> - '.__('The HTML tags or CSS selectors you\'d like this font to apply to (more info in the "CSS Selectors" help tab). You can specify multiple selectors separated by comma\'s. Ex: h1, #some_id, .some_class', 'myportfolio' ).'</li></ul>';
		$selectors_title 			= __('CSS Selectors', 'myportfolio' );
		$selectors_content		= '<p>' . __('CSS Selectors are used to hook your font rows into your actual website. Once you\'ve added, customized, and defined CSS selectors for your fonts, Google Typography will automatically insert all the necessary CSS into your website.', 'myportfolio' ) . '</p>';
		$selectors_content		= '<p>' . __('Here are some examples of the selectors you can use:', 'myportfolio' ) . '</p>';
		$selectors_content	 .= '<ul><li><b>'.__('IDs', 'myportfolio' ).':</b> '.__('#selector', 'myportfolio' ).'</li><li><b>'.__('Classes:', 'myportfolio' ).':</b> '.__('.selector', 'myportfolio' ).'</li><li><b>'.__('HTML Tags', 'myportfolio' ).':</b> '.__('span', 'myportfolio' ).'</ul>';
		$selectors_content	 .= '<p><b>'.__('Example', 'myportfolio' ).':</b> '.__('#selector span.date', 'myportfolio' ).'</p>';
	...
	function plugin_link($links, $file) {
		if ( $file == 'google-typography/google-typography.php' ) {
			$links['settings'] = sprintf( '<a href="%s"> %s </a>', admin_url( 'themes.php?page=typography' ), __( 'Settings', 'myportfolio' ) );
		}
		return $links;
	}
	...
	function options_ui() {
		...
		$title						= __('Theme Typography', 'myportfolio' );
		$loading					= __('Loading Your Collections', 'myportfolio' );
		$add_new					= __('Add New Collection', 'myportfolio' );
		$reset						= __('Reset Collections', 'myportfolio' );
		$preview_text   			= __('Type in some text to preview...', 'myportfolio' );
		$collection_title 			= __('New Collection', 'myportfolio' );
		//$this->preview_text = $preview_text; // to pass it to JS
		$preview_hint   			= __('Preview Background Color', 'myportfolio' );
		//$font_family_title  	= __('Font family...', 'myportfolio' );
		$std_font_family_title  	= __('Standard Fonts', 'myportfolio' );
		$google_font_family_title  	= __('Google Fonts', 'myportfolio' );
		$font_variant_title 		= __('Variant...', 'myportfolio' );
		$font_size_title 			= __('Size...', 'myportfolio' );
		$backup_font_title			= __('Fallback:', 'myportfolio' );
		$css_selectors_title		= __('CSS Selectors:', 'myportfolio' );
		$additional_css_title		= __('Additional CSS:', 'myportfolio' );
		$delete_button_text			= __('Delete', 'myportfolio' );
		$save_button_text			= __('Save', 'myportfolio' );
		
		$welcome_title 				= __('Theme Typography', 'myportfolio' );
		$welcome_subtitle			= __('Get started in 3 steps.', 'myportfolio' );//.'<a href="https://vimeo.com/67957799" target="_blank">'.__('Watch the video tutorial &#x2192;', 'myportfolio' ).'</a>';
		$step_1_title 				= __('1. Pick A Font', 'myportfolio' );
		$step_1_desc				= __('Choose from any of the 600+ Google Fonts.', 'myportfolio' );
		$step_2_title 				= __('2. Customize It', 'myportfolio' );
		$step_2_desc				= __('Pick a size, variant, color and more.', 'myportfolio' );
		$step_3_title 				= __('3. Attach It', 'myportfolio' );
		$step_3_desc				= __('Attach your font to any CSS selector(s).', 'myportfolio' );
		$year						= date("Y");
		$themename = MTS_THEME_NAME;
		
		$character_sets_title = __('Character sets', 'myportfolio' );
		$character_sets_desc = __('Choose the character sets you wish to include. Please note that not all sets are available for all fonts.', 'myportfolio' );
		...
		$character_sets = array(
			'latin' 		=> __('Latin', 'myportfolio' ),
			'latin-ext' 	=> __('Latin Extended', 'myportfolio' ),
			'cyrillic' 		=> __('Cyrillic', 'myportfolio' ),
			'cyrillic-ext'	=> __('Cyrillic Extended', 'myportfolio' ),
			'greek'			=> __('Greek', 'myportfolio' ),
			'greek-ext'		=> __('Greek Extended', 'myportfolio' ),
			'vietnamese'	=> __('Vietnamese', 'myportfolio' ),
			'khmer'			=> __('Khmer', 'myportfolio' ),
			'devanagari'	=> __('Devanagari', 'myportfolio' ),
		);
...
EOT;
	...
	function admin_scripts() {
		...
		wp_localize_script('google-typography', 'googletypography', array(
				'reset_confirm' => __("Are you sure you want to revert to the theme's default collections? Note: You will lose any custom collections you've created.", 'myportfolio' ),
				'delete_confirm' => __("Are you sure you want to delete this collection?", 'myportfolio' )
			));
