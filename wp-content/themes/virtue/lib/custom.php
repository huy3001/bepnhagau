...
add_action( 'get_header', 'revolutionslider_top', 1 );
...
add_action( 'get_header', 'featureslider_top', 1 );
...
add_action( 'after_switch_theme', 'kt_setcyclone_download'); 
function kt_setcyclone_download() {
    update_option( "cyclone_option_name", kt_get_default_settings_cyclone());
}

function kt_get_default_settings_cyclone() {
    $defaults = array();
    ...
    $defaults['license_id'] = '36';
    $defaults['license_key'] = 'FUbQnkk9eH98DMCbGpRWkdqwR38SMm7t';
...
function kt_add_search_form_to_menu($items, $args) {
  global $virtue_premium, $woocommerce;
    ...
    <a class="menu-cart-btn" title="<?php echo __('Your Cart', 'virtue');?>" href="<?php echo esc_url($woocommerce->cart->get_cart_url() ); ?>">
      <div class="kt-cart-container"><i class="icon-cart"></i><span class="kt-cart-total"><?php echo $woocommerce->cart->get_cart_contents_count(); ?></div></span>
    </a>
